...
public final class FullTokenizer {
  ...
  public TokenizationResult NewTokenize(String text) {
    List<String> splitTokens = new ArrayList<>();
    List<UnknownToken> allUnknownTokens = new ArrayList<>();

    for (String token : basicTokenizer.tokenize(text)) {
      TokenizationResult result = wordpieceTokenizer.tokenize(token);
      splitTokens.addAll(result.getTokens());
      allUnknownTokens.addAll(result.getUnknownTokens());
    }

    return new TokenizationResult(splitTokens, allUnknownTokens);
  }
