...
public final class WordpieceTokenizer {
  ...
  private List<UnknownToken> unknownTokens = new ArrayList<>();
  ...
  public TokenizationResult tokenize(String text) {
    ...
    for (String token : BasicTokenizer.whitespaceTokenize(text)) {
      ...
      if (token.length() > MAX_INPUTCHARS_PER_WORD) {
        ...
        // memorăm cuvântul original aici la poziția curentă
        unknownTokens.add(new UnknownToken(outputTokens.size() - 1, token));
        continue;
      }
    ...
    return new TokenizationResult(outputTokens, unknownTokens);
  }
}
